...
/*exports.mostCategory =function(req, res, next){
		req.getConnection(function(err, connection){
		   connection.query('SELECT categories.category_name,SUM(sales_date)AS qty FROM sales INNER JOIN products ON sales.category_id = categories.category_id INNER JOIN categories ON products.category_id = categories.category_id GROUP BY categories.category_name ORDER BY SUM(sales.qty)DESC LIMIT 0,1', [], function(err, rows){
    		   if (err) next(err);
